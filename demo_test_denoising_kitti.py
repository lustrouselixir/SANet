...
def main():
    ...
    # Load model
    model = SANet().cuda()
    model.StrAgg.load_state_dict(torch.load('models/structure_aggregation_kitti.pkl'))
    model.load_state_dict(torch.load('models/SANet_kitti.pkl'))
    model.eval()
    lpfunc = lpips.LPIPS(net='vgg').cuda()

    # Load Image - KITTI Stereo 2015
    target_img = cv2.imread('examples/KITTI/01_target.png')
    target_img = cv2.cvtColor(target_img, cv2.COLOR_BGR2RGB)
    target_img = np.array(target_img/255.0, dtype=float)

    guidance_img = cv2.imread('examples/KITTI/01_guidance.png')
    ...
    im.save('results/Denoising/01_kitti_res.png')

    im = Image.fromarray(np.uint8(noisy_img* 255))
    im.save('results/Denoising/01_kitti_noisy.png')
