...
def main():
    ...
    # Load Image - Ours
    target_img = cv2.imread('examples/Ours/01_target.png')
    target_img = cv2.cvtColor(target_img, cv2.COLOR_BGR2RGB)
    target_img = np.array(target_img/255.0, dtype=float)

    guidance_img = cv2.imread('examples/Ours/01_guidance.png', cv2.IMREAD_GRAYSCALE)
    guidance_img = np.expand_dims(np.array(guidance_img/255.0, dtype=float), 2)
    ...
    im.save('results/Denoising/01_ours_res.png')

    im = Image.fromarray(np.uint8(noisy_img* 255))
    im.save('results/Denoising/01_ours_noisy.png')
